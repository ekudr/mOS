...
//    unsigned int min_clk;
    ...
    int          buswidth;
    uint32_t	 voltages;
    uint32_t fifoth_val; /* use fifo mode to read and write data */
	int fifo_mode;
...
struct mmc_ops {
    ...
    int (*set_ios)(struct mmc_host *host);
    int (*send_cmd)(struct mmc_host *host, mmc_cmd_t *cmd, mmc_data_t *data);
	
	int (*getcd)(struct mmc_host *host);
	int (*getwp)(struct mmc_host *host);
	int (*host_power_cycle)(struct mmc_host *host);
	int (*get_b_max)(struct mmc_host *host, void *dst, uint64_t blkcnt);
};
